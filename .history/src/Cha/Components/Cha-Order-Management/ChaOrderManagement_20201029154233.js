...
const setTabActive = (addElem, removeName) => {
  ...
  const TabMenu = () => {
    ...
    const tabContentA = (e) => {
      ...
      setComponent(<ComponentA ="Hi" />);
    };

    const tabContentB = (e) => {
      setTabActive(e.target, '.jess-productTabMenu__item');
      setComponent(<ComponentB ="B" />);
    };
    const tabContentC = (e) => {
      setTabActive(e.target, '.jess-productTabMenu__item');
      setComponent(<ComponentC ="B" />);
    };
    const tabContentD = (e) => {
      setTabActive(e.target, '.jess-productTabMenu__item');
      setComponent(<ComponentD ="B" />);
    };
    return (
      <>
        <div className="cha-order-mana-content-container col-9">
          <div className="cha-order-mana-content-row1">
            <div className="jess-productTabMenu__item" onClick={tabContentA}>
              未送達
            </div>
            <div className="jess-productTabMenu__item" onClick={tabContentB}>
              已送達
            </div>
            <div className="jess-productTabMenu__item" onClick={tabContentC}>
              已退費
            </div>
            <div className="jess-productTabMenu__item" onClick={tabContentD}>
              揪團中
            </div>
          </div>
