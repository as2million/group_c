...
function ChaOrderManagement(props) {
  ...
  const TabMenu = () => {
    ...
    async function getMyOrderAndMyOderDetail(paramsMemberId) {
      ...
      // 訂單小計
      setSubtotalPriceDetail(dataAllOrder[44].order_detail[0].subtotal_price);
      // 怪獸幣
      setBeastieCoinDetail(dataAllOrder[44].order_detail[0].beastie_coin);
      // 運費
    ...
    return (
      <>
        <div className="cha-order-mana-content-container col-9">
          ...
          <h1>訂單總額：{productPriceMyOrder}</h1>
        </div>
      </>
    );
  };
