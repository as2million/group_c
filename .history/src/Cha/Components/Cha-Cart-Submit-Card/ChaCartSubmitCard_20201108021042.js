...
function ChaCartSubmitCard(props) {
  ...
  let myOrderDetailItem = {};
  let myOrderDetailArray = mealsDisplay.map(
    (item, index) =>
      (myOrderDetailItem = {
        order_sid: 1,
        product_sid: item.id,
        product_amount: item.productAmount,
        product_name: amount.productName,
        product_price: productPrice,
      })
  );

  // POST給my_order的資料
  async function updateToMyOrderDetail() {
    const myOrderDetailArray = {
      order_sid: 1,
      product_sid: 111,
      product_amount: 11,
      product_name: '玫瑰岩鹽烤雞',
