...
function ChaOrderManagement(props) {
  ...
  let notArrived = [],
    ...
    Refund = [],
    Grouping = [];
  const result = orderData.map((item, index) => {
    if (item.order_state === '未送達') {
      notArrived.push(item);
    } elseif (item.order_state === '已送達') {
      Arrived.push(item);
    } elseif (item.order_state === '已退費/已取消') {
      Refund.push(item);
    } elseif (item.order_state === '揪團中') {
      Grouping.push(item);
    }
  });
  // 未達成
