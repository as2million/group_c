...
function ChaOrderManagement(props) {
  ...
  const TabMenu = () => {
    ...
    const ComponentNotArrived = (e) => {
      setTabActive(e.target, '.jess-productTabMenu__item');
      setComponent(<ComponentNotArrived />);
    };

    const ComponentArrived = (e) => {
      setTabActive(e.target, '.jess-productTabMenu__item');
      setComponent(<ComponentArrived />);
    };
    return (
      <>
