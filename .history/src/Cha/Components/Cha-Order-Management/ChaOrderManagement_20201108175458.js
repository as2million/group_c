...
function ChaOrderManagement(props) {
  ...
  const TabMenu = () => {
    ...
    async function getMyOrder() {
      ...
      // setName(data[0].name);
      setNameMyOrder(dataMyOrder[32].take_person);
      setMobileMyOrder(dataMyOrder[32].mobile);
      // setTakeWay(data[32].take_way);
      setAddressMyOrder(dataMyOrder[32].take_address);
      // setMemberSidMyOrder(dataMyOrder[32].sid);
      setMemberSidMyOrder(dataMyOrder[32].sid);
      setBeastieCoinMyOrder(dataMyOrder[32].beastie_coin);
      setOrderStateMyOrder(dataMyOrder[32].order_state);
      getMyOrderDetail(dataMyOrder[32].sid);
    }
    // // GET會員資料
    // async function getMyOrder() {
    //   const url = 'http://localhost:5000/cart-api/my-order';
