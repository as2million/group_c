...
function ChaOrderManagement(props) {
  ...
  const TabMenu = () => {
    const [component, setOrderomponent] = useState(<ComponentA />)

    const tabContentA = (e) => {
      setTabActive(e.target, '.cha-order-mana-title-switch')
      setOrderomponent(<ComponentA />)
    }

    const tabContentB = (e) => {
      setTabActive(e.target, '.cha-order-mana-title-switch')
      setOrderomponent(<ComponentB />)
    }
    const tabContentC = (e) => {
      setTabActive(e.target, '.cha-order-mana-title-switch')
      setOrderomponent(<ComponentC />)
    }
    const tabContentD = (e) => {
      setTabActive(e.target, '.cha-order-mana-title-switch')
      setOrderomponent(<ComponentD />)
    }
    return (
      <>
